...
path_data = '../data/Rolling_sample_L2/'
...
def Classification_Distances(Reference , Test):
    ...
    Ref, Tree = initialize_calculation(T = None, Data = scaler.transform(Reference), gsize = 2,\
    par_train = 0, output_dimension = 4)
    T, Tree = initialize_calculation(T = Tree, Data = scaler.transform(Test), gsize = 2,\
    par_train = 1, output_dimension = 4 )
    print "Distances",Ref.shape, T.shape
...
def classification_start(C, dataset_type, dataset_num):
    ...
    for i in xrange(1,C+1):
        ...
        R.append(Temp)
    R = np.array(R)
    X_train, y_train, X_test, y_test = load_data_pickle(dataset='rolling')
    T  = X_test
    y_true  = y_test
    print T.shape
    print R.shape
    ...
    T, y_pred = Classification_Faults(R, T, y_true, C)

    print max(y_pred), max(y_test)
